/*
 * npx terser -c -m -o ztree.min.js  ./js/jquery.ztree.all.js  
 * JQuery zTree core
 * v3.5.44
 * http://treejs.cn/
 *
...
(function ($) {
  var settings = {}, roots = {}, caches = {},
    ...
    data = {
      ...
      nodeIconFont: function (setting, node, newName) {
        var key = setting.data.key.iconfont;
        if (typeof newName !== 'undefined') {
          node[key] = newName;
        }
        return node[key];
    ...
    view = {
      addNodes: function (setting, parentNode, index, newNodes, isSilent) {
        ...
        if (parentNode) {
          ...
          if (!parentNode.open) {
            if(!data.nodeIconFont(setting,parentNode)){
      ...
      expandCollapseNode: function (setting, node, expandFlag, animateFlag, callback) {
        ...
        if (isParent) {
          ...
          if (node.open) {
            if(!data.nodeIconFont(setting,node)){
      ...
      makeDOMNodeIcon: function (html, setting, node) {
        var nameStr = data.nodeName(setting, node),
          iconfont =data.nodeIconFont(setting, node),
          iconfont =iconfont? iconfont : view.makeNodeIcoClass(setting, node),
          style = iconfont? '' : view.makeNodeIcoStyle(setting, node),
          name = setting.view.nameIsHTML ? nameStr : nameStr.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        html.push("<span id='", node.tId, consts.id.ICON,
          "' title='' treeNode", consts.id.ICON, " class='", iconfont,
    ...
    _view = {
      ...
      moveNode: function (setting, targetNode, node, moveType, animateFlag, isSilent) {
        ...
        if (!setting.data.keep.parent && oldChildren.length < 1) {
          ...
          if(!data.nodeIconFont(setting,oldParentNode)){
            view.replaceIcoClass(oldParentNode, tmp_icoObj, consts.folder.DOCU);
          }
          tmp_ulObj.css("display", "none");
