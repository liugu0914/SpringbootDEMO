...
public class OAuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        User user = (User) redisUtil.hget(UserConstant.USER, token.getUserId());
        ...
        if (subject.isAuthenticated()) {
            ...
            if (!permitted) {
                log.warn("账号[{}]没有[{}]的操作权限", user.getAccount(), uri);
                throw new ServerException(StateCode.NOPERMISSION);
            }
        } else {
            log.warn("账号[{}]没有认证登陆", user.getAccount());
